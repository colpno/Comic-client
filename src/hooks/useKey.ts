...
type Keys = ModifierKeys | WhitespaceKeys | NavigationKeys | FunctionKeys | NumericKeypadKeys | UIKeys;

export const useKey = (keys: Keys[], callback: () => void) => {
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (keys.length > 0 && keys.includes(event.key as Keys)) {
        callback();
      }
    };
